...
palitrs = {
    ...
    "red" : "#FF0700	#BF3330	#A60400	#FF4540	#FF7673",
    "soft" : "#f9f1f0 #FFAEBC #A0E7E5 #B4F8C8 #FBE7C6",
    "orange" : "#FF7100	#BF6F30	#A64A00	#FF9540	#FFB173",
...
try:
    inf = open(cf.colorthemeFile).read()
    inf = inf if inf != "" else "gold"
    palitr = list(palitrs[inf].split())
except:
    file = open(cf.colorthemeFile, "w",encoding="utf-8")
    file.write("gold")
    file.close()
    palitr = list(palitrs[open(cf.colorthemeFile, encoding="utf-8").read()].split())

def newTheme(color):
    a = True 
    if cf.askQuestionWhenTheme:
        if ms.askokcancel(message="Перезапустите программу"):
            a = True 
        else:
            a = False 
    if a:
        with open(cf.colorthemeFile, "w") as file:
            file.write(color)
...
class gui():
    ...
    def makewidgets(self):
        ...
        self.text = Button(self.root, bg = config.wind_background, relief = "flat", fg = "black", text = "Открыть гугл-таблицы", command = lambda a = 0: webbrowser.open(cf.SheetsLink, 2), activebackground = config.buttons_bg, activeforeground = config.buttons_fg)
    ...
    def makeMenu(self):
        ...
        self.themesmenu = Menu(self.mainmenu, tearoff=0, background = colors.five, foreground = colors.three)
        # Filemenu
        ...
        # ThemesMenu
        for i in palitrs:
            self.themesmenu.add_command(label=i, command = lambda color = i:newTheme(color), background = list(palitrs[i].split())[4], foreground = list(palitrs[i].split())[2])
